...
def generate_custom_password():
    # केवल alphabets और numbers
    characters = string.ascii_letters + string.digits
    
    # प्रत्येक random part की length 4-8 characters
    len1 = random.randint(4, 8)
    len2 = random.randint(4, 8)
    len3 = random.randint(4, 8)
    
    # Random parts generate करें
    random1 = ''.join(random.choice(characters) for _ in range(len1)).upper()
    random2 = ''.join(random.choice(characters) for _ in range(len2)).upper()
...
def home():
    return jsonify({
        ...
        "password_format": "{RANDOM}_{RANDOM}_BY_DANGER_{RANDOM} (letters and numbers only)"
    })
